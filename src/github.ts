...
export class GitHubService {
  ...
  async getTargetBranch(): Promise<string> {
    ...
    const configuredTargetBranch =
      dispatchTargetBranch || getInput('target_branch');
   ...
   */
  async hasArtifactsForCommit(
    commitHash: string,
    branch?: string,
  ): Promise<boolean> {
    try {
      const workflowRuns = await this.findAllWorkflowRunsByCommit(
        commitHash,
        'completed',
        branch,
      );
      ...
      for (const workflowRun of workflowRuns) {
        try {
          const runArtifacts = await this.listArtifactsForWorkflowRun(
            workflowRun.id,
          );
  ...
  async getParentCommit(commitHash: string): Promise<string | null> {
    ...
    try {
      ...
      if (
        commitResponse.data.parents &&
        commitResponse.data.parents.length > 0
      ) {
      ...
      const apiError = error as ApiError;
      console.warn(
        `⚠️  Failed to get parent commit for ${commitHash}: ${apiError.message}`,
      );
      return null;
    }
  }
  ...
  async getTargetBranchLatestCommit(): Promise<{
    ...
    try {
      ...
      console.log(
        `✅ Found commit hash from GitHub API: ${formatShortSha(latestCommitHash)}`,
      );
      ...
      console.log(
        `🔍 Checking if commit ${latestShortHash} has baseline artifacts...`,
      );
      const hasArtifacts = await this.hasArtifactsForCommit(
        latestCommitHash,
        targetBranch,
      );
      ...
      console.log(
        `⚠️  Commit ${latestShortHash} does not have baseline artifacts`,
      );
      ...
      for (let depth = 0; depth < maxDepth; depth++) {
        ...
        if (!parentCommit) {
          console.log(
            `⚠️  Reached the beginning of the branch, no more parent commits`,
          );
          break;
        }
        ...
        const parentHasArtifacts = await this.hasArtifactsForCommit(
          parentCommit,
          targetBranch,
        );
        ...
        if (parentHasArtifacts) {
          console.log(
            `✅ Found commit ${parentShortHash} with baseline artifacts`,
          );
          console.log(
            `\n⚠️  Note: The latest commit (${latestShortHash}) does not have baseline artifacts.`,
          );
          console.log(
            `   Using commit ${parentShortHash} for baseline comparison instead.`,
          );
          console.log(
            `   If this seems incorrect, please wait a few minutes and try rerunning the workflow.`,
          );
          return {
      ...
      console.log(
        `   Note: If baseline comparison fails, please wait a few minutes and try rerunning the workflow.`,
      );
      return {
        commitHash: latestCommitHash,
        usedFallbackCommit: false,
      ...
      const apiError = error as ApiError;
      throw new Error(
        `Failed to get target branch (${targetBranch}) commit: ${apiError.message}`,
      );
    }
  }
  ...
  async findWorkflowRunByCommit(
    ...
    try {
      ...
      if (
        ...
        runsResponse.data.workflow_runs.length > 0
      ) {
      ...
      const matchingRun = allRunsResponse.data.workflow_runs?.find(
        (run: any) =>
          run.head_sha.startsWith(shortHash) ||
          run.head_sha.startsWith(commitHash),
      );
      ...
      const apiError = error as ApiError;
      console.warn(
        `⚠️  Failed to find workflow run for commit ${commitHash}: ${apiError.message}`,
      );
      return null;
    }
  }
  ...
  async findAllWorkflowRunsByCommit(
    ...
    try {
      ...
      const matchingRuns =
        allRunsResponse.data.workflow_runs?.filter(
          (run: any) =>
            run.head_sha.startsWith(shortHash) ||
            run.head_sha.startsWith(commitHash),
        ) || [];
      ...
      const apiError = error as ApiError;
      console.warn(
        `⚠️  Failed to find workflow runs for commit ${commitHash}: ${apiError.message}`,
      );
      return [];
    }
  }
  ...
  async listArtifactsForWorkflowRun(runId: number) {
    ...
    try {
      ...
      const apiError = error as ApiError;
      console.warn(
        `⚠️  Failed to list artifacts for workflow run ${runId}: ${apiError.message}`,
      );
      throw error;
    }
  }
  ...
  async findArtifactByNamePattern(pattern: string): Promise<Artifact | null> {
    ...
    console.log(
      `Available artifacts: ${artifacts.artifacts.map((a: Artifact) => a.name).join(', ')}`,
    );
    ...
    if (matchingArtifacts.length > 0) {
      console.log(
        ...
        matchingArtifacts.map((a: Artifact) => a.name),
      );
      return matchingArtifacts.sort(
