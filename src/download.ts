  ...
  return new Promise((resolve, reject) => {
    ...
    const finish = (error?: Error, content?: Buffer) => {
      ...
      if (error) {
        ...
        reject(
          new Error(`Target file ${fileName} could not be read from artifact`),
        );
      }
    };
    ...
    const timeout = setTimeout(() => {
      finish(
        new Error(
          `Timed out after ${timeoutMs}ms while extracting ${fileName}`,
        ),
      );
    }, timeoutMs);
...
export async function downloadArtifactByCommitHash(
  ...
  console.log(
    `📋 Searching for artifact with path hash and commit hash: ${expectedArtifactName}`,
  );
  ...
  const workflowRuns =
    await githubService.findAllWorkflowRunsByCommit(commitHash);
  ...
  if (workflowRuns && workflowRuns.length > 0) {
    console.log(
      `✅ Found ${workflowRuns.length} workflow run(s) for commit ${commitHash}`,
    );
    ...
    for (const workflowRun of workflowRuns) {
      ...
      console.log(
        `   Status: ${workflowRun.status}, Conclusion: ${workflowRun.conclusion}`,
      );

      try {
        const runArtifacts = await githubService.listArtifactsForWorkflowRun(
          workflowRun.id,
        );
        const foundArtifact = runArtifacts.artifacts?.find((a: any) =>
          compatibleArtifactNames.has(a.name),
        );
        ...
        if (foundArtifact) {
          ...
          console.log(
            `✅ Found artifact in workflow run ${workflowRun.id}: ${artifact.name} (ID: ${artifact.id})`,
          );
          break; // Found it, stop searching
        } else {
          const artifactNames =
            runArtifacts.artifacts?.map((a: any) => a.name).join(', ') ||
            'none';
          console.log(
            `   ⚠️  Artifact not found. Available artifacts: ${artifactNames}`,
          );
        }
      } catch (runArtifactsError) {
        console.warn(
          `   ⚠️  Failed to get artifacts from workflow run ${workflowRun.id}: ${runArtifactsError}`,
        );
  ...
  if (!artifact) {
    ...
    artifact = artifacts.artifacts.find((a: any) =>
      compatibleArtifactNames.has(a.name),
    );
  }
  ...
  if (!artifact) {
    ...
    if (artifacts?.artifacts) {
      console.log(
        `   Available artifacts: ${artifacts.artifacts.map((a: any) => a.name).join(', ')}`,
      );
    }
